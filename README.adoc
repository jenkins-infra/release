...
The first part that we mention by using the term the *release*, is when we create a new java code release. It relies on the Maven Release Plugin to perform the release. It involves signing with a GPG key. At the end of this operation signed maven artifacts are pushed to a Maven repository.
...
==== Stage
Packages can be staged privately.
More information is available in link:https://github.com/jenkins-infra/helpdesk/issues/1110[INFRA-1363] and link:https://github.com/jenkins-infra/helpdesk/issues/2193[INFRA-2608]
...
* https://github.com/jenkins-infra/helpdesk/issues/663[INFRA-910] - EPIC for the Jenkins Core Release Environment
