...
function packaging() {
	...
	## TODO: update "latest" symlinks - https://github.com/jenkins-infra/jenkins-infra/blob/production/dist/profile/templates/mirror-scripts/update-latest-symlink.sh.erb
...
function promotePackages() {
	...
	## Step 2/3 - Copy binaries and HTML from staging to (local NFS) get.jenkins.io
	pushd "${get_jenkins_io_staging}"
	rsync --recursive \
		--links `# Copy symlinks as symlinks: destination is a Linux filesystem` \
		--perms `# Preserve permissions: destination is a Linux filesystem` \
	...
	## Step 3/3 - Copy package sites from staging to (remote) pkg.jenkins.io
